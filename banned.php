...
<head>
	...
	<title><?php echo $Holo['name']; ?>: Baneado</title>
...
<section>
	...
	<div class="text-center"> 
		...
		<p class="text-muted mb-0">Su cuenta fue baneada de <?php echo $Holo['name']; ?> Hotel.</p>
		<hr>
		<p class="text-muted mb-0">Baneado el dia <?php echo date('d/m', $chbe['timestamp']); ?> del <?php echo date('Y', $chbe['timestamp']); ?> hasta <?php echo date('d/m', $chbe['ban_expire']); ?> del <?php echo date('Y', $chbe['ban_expire']); ?>.</p>
<?php
$staffs = mysql_query("SELECT * FROM users WHERE id = '".$chbe['user_staff_id']."'");
while($staff = mysql_fetch_assoc($staffs)){
?>
		<p class="text-muted mb-0">Baneado por <?php echo $staff['username']; ?>.</p>
<?php } ?>
		<hr>
