...
<?php } ?>
					...
					<hr>
					<div class="mb-1 text-muted"><font color="#FF9030"><strong><?php echo mysql_real_escape_string($usr->credits); ?></strong> Creditos</font></div>
		...
		<div class="col-md-8 pl-md-3">
			<section class="py-2">
				<div class="tab-content" id="tabsContent">
                        ...
                        <div class="row">
							<div class="card">
								<div class="card-body text-center text-muted">
									<strong><?php echo mysql_real_escape_string($usr->username); ?></strong> Se unio a <?php echo $Holo['name']; ?> el <strong><?php echo date("d/m",mysql_real_escape_string($usr->account_created)); ?></strong>, <strong><?php echo date("Y",mysql_real_escape_string($usr->account_created)); ?></strong>, a recibido <strong><?php echo $user_n['respects_received']; ?></strong> Respetos, pero también dado <strong><?php echo $user_n['respects_given']; ?></strong> Respetos, también te informamos que <?php echo mysql_real_escape_string($usr->username); ?> tiene <strong><?php echo $user_n['achievement_score']; ?></strong> Puntos de logros, y a comprado <strong><?php echo mysql_num_rows(mysql_query("SELECT * FROM items WHERE user_id = '".mysql_real_escape_string($usr->id)."'")) ?></strong> Furnis.
								</div>
							</div>
						</div>
					</div>
					...
					<h3 class="mb-3"><?php echo mysql_real_escape_string($usr->username); ?> tiene <?php echo mysql_num_rows(mysql_query("SELECT * FROM rooms WHERE owner_id = '".mysql_real_escape_string($usr->id)."'")) ?> Salas</h3>
						<div class="row">
...
<?php } ?>
					...
					<h3 class="mb-3"><?php echo mysql_real_escape_string($usr->username); ?> tiene <?php echo mysql_num_rows(mysql_query("SELECT * FROM users_badges WHERE user_id = '".mysql_real_escape_string($usr->id)."'")) ?> Placas</h3>
						<div class="row">
...
<?php } ?>
					...
					<h3 class="mb-3"><?php echo mysql_real_escape_string($usr->username); ?> tiene <?php echo mysql_num_rows(mysql_query("SELECT * FROM camera_web WHERE user_id = '".mysql_real_escape_string($usr->id)."'")) ?> Fotos</h3>
						<div class="row">
